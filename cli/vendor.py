#!/usr/bin/env python3
"""vendor.py — copy the published verifier, keys, expected-config files and pinned TSA trust anchors from the repo root
into notbefore/verifier/, pinning them to the repo commit. Re-run (and bump the package version) whenever the protocol,
keys or trust anchors change. Nothing is downloaded."""
...
FILES += sorted("keys/tsa/" + f for f in os.listdir(os.path.join(ROOT, "keys", "tsa")))      # pinned RFC 3161 trust anchors (ERR-014)
CERTS = {}   # nothing is downloaded at vendoring time any more: the trust anchors are tracked files under keys/tsa/
