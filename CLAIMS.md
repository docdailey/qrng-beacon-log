...
| **Precision vs accuracy:** "~8–10 ns RMS discipline over the stated window" | the statement's measured figures | per pulse | authenticated; checked against `notbefore/timing/v1` (0.13.0) or v2 (0.15.0, adds cadence provenance) when a contract declares it; the CLI declares v2 by default | **never** a nanosecond figure as accuracy vs UTC; absolute budget is uncalibrated except the **measured** 69 ns antenna delay; L1-only today (ionospheric term uncorrected); dual-band is roadmap |
| **Two GNSS paths, one mesh:** "the i210 (F9T-disciplined) continuously measures the BMC grandmaster (LEA-6T-disciplined); k3 is disciplined by that BMC and signs its own clock observations" | `mesh_crosscheck` in the time statement; `time_witness` statement | statements signed from 0018; mesh field per `stamp_probe.py` | authenticated (signature, pulse binding); enforced against `notbefore/timing/v1` or v2 when a contract declares it (0.13.0 / 0.15.0), reported otherwise | describe as **measure and trim** (the i210 monitor is `free_running`; the BMC carries the `ptptgt` trim); k3 is a **same-sponsor downstream participant**, not an independent reference; both paths start at GPS |
...
| **Host attestation & isolation:** "each host signs its own statement; a compromised aggregator cannot fabricate a host's facts (confined OS user, fixed-role forced command until 2026-09-13, then `beacon-agentd`: signed requests over TCP, no shell, no session)" | v0.5 statements; `hosts/ISOLATION.md`; `execution` self-reports enforced (`via_forced_command` or `via: agentd` against pinned hashes) | statements 0018; isolation 0020; enforced self-reports 0026; agentd from the k3 cutover seq | verify.py strict path (`notbefore` ≥ 0.14.0 for agentd statements) | the *operator* (all machines) is out of scope of this property; 0018/0019 are KNOWN-NONCOMPLIANT (ERR-007) |
...
| **Aggregator host:** "k3 assembles and signs pulses from 0098 (think did from 0018 to 0097); k3 also hosts the time witness, in a separate confined user" | `signatures.aggregator.signer`, `core.aggregator_host`; `keys/KEYS.json` validity windows; `hosts/ISOLATION.md` | 0098 | `verify.py --pin` requires the aggregator key valid at the pulse seq | **say alongside:** the witness and the aggregator share a host — a host-level compromise of k3 gains both; the witness is a same-sponsor downstream participant either way (see "Two GNSS paths") |
| **Cadence source:** "k3, the aggregator, declares and starts the hour at :00:00.000 UTC on its own PTP-disciplined clock (`core.cadence.self_trigger`, `start_source = own-clock` from 0110; on the arrival of p550's datagram 0098–0108); p550's i210-disciplined clock corroborates the same instant with a signed statement carrying its hardware second event (`core.cadence.trigger`, `hw_event`), bound into the commit; the target round is the round released at that instant + lead (100 through 0101, 20 from 0102), so releases are at :01:00 (were :05:00)" | `core.cadence.self_trigger` (aggregator wake record) and `core.cadence.trigger` (a `time_attester` statement: `scheduled_unix_s`, `wake.how`, `hw_event.edge_after_instant_ns`, PHC−REALTIME at wake); live figures: k3 +19–23 µs, p550 edge +21–37 µs, statement at k3 +1.1–1.3 ms (0110–0118) | 0098 (self-trigger record); 0110 (own-clock start) | `verify.py` checks the trigger's signature under p550's pinned key, that both records name the same instant, that it is a round boundary and that `target_release == instant + lead×3`; **`notbefore/timing/v2` (0.15.0, spec 0.12) enforces the provenance**: both records present, same instant, hardware event (not clock fallback), bounded edge/issue/receive latency, timely statements; a missing or rejected trigger is NOT-SATISFIED. Under v1 the trigger is authenticated but its absence is not a failure | say **"declared by k3's clock, corroborated by p550's hardware event"**, never "started by the hardware event" for 0110+ (that was 0098–0108) and never "triggered by p550" for 0098+; the two paths share GPS as the ultimate source; a pulse without a bound trigger is honest about it (`cadence.source = "k3 clock"`) and fails v2 |
...
| "hourly on think's timer" (before 2026-09-13) vs "started by p550's clock" (0092–0097) vs "started on the arrival of p550's hardware-event datagram" (0098–0108) vs **"declared by k3's own clock, corroborated by p550's hardware event" (0110+)** | **From 0098 the aggregator is k3.** 0092–0097: think started on p550's SSH-delivered trigger (0096 failed at the reveal, ERR-017). 0098–0108: k3 waited at the instant and started on the datagram. From 0110 k3 starts on its own clock (18.9–23 µs late) and binds the datagram as attestation. A fallback pulse says `core.cadence.source = "<host>-timer"`. The eras are distinguishable from the pulse (`self_trigger.start_source`) and must not be conflated in copy. |
...
- ✅ "Every v0.5 commit carries at least two RFC 3161 tokens taken at mint, each ≥ 120 s before its round" (through 0101; ≥ 20 s from 0102, when the lead became 60 s - CADENCE.md).
...
**Boundary ruling (2026-09-14, from review):** the physical chain makes accidental or casual fabrication difficult; a malicious operator
controlling every host and signing key can still fabricate the summaries the hosts sign. Roadmap that moves that boundary, in order:
publish per-cycle **raw edge-record digests** (the i210 EXTTS stream and `pps1` assert log around the instant, hashed into the time
statement) so a summary can be checked against its source records; add an **independent capture device** (a second timestamper on
the same pulses under different administration) and an **outside witness** of the cadence. Until then the copy says
"authenticated and checked against a declared profile", not "independently observed".
