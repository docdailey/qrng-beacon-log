...
| "hourly on think's timer" (before 2026-09-13) vs "started by p550's clock" (0092–0097) vs "started on k3's own clock" (0098+; on the arrival of p550's hardware-event datagram 0098–0108, on k3's clock with that datagram bound as attestation from 0110) | **From 0098 the aggregator is k3 and the instant is its own clock, corroborated by p550's UDP-delivered signed trigger.** 0092–0097: think started on p550's SSH-delivered trigger (0096 failed at the reveal, ERR-017). A fallback pulse says `core.cadence.source = "<host>-timer"`. |
