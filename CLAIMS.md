...
| **Aggregator host:** "k3 assembles and signs pulses from 0096 (think did from 0018 to 0095); k3 also hosts the time witness, in a separate confined user" | `signatures.aggregator.signer`, `core.aggregator_host`; `keys/KEYS.json` validity windows; `hosts/ISOLATION.md` | 0096 | `verify.py --pin` requires the aggregator key valid at the pulse seq | **say alongside:** the witness and the aggregator share a host — a host-level compromise of k3 gains both; the witness is a same-sponsor downstream participant either way (see "Two GNSS paths") |
...
| "hourly on think's timer" (before 2026-09-13) vs "started by p550's clock" (0092–0095) vs "started on k3's own clock" (0096+) | **From 0096 the aggregator is k3 and the instant is its own clock, corroborated by p550's UDP-delivered signed trigger.** 0092–0095: think started on p550's SSH-delivered trigger. A fallback pulse says `core.cadence.source = "<host>-timer"`. |
