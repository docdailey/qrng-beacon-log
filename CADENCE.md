...
40 ns: ts2phc locked (s2), the BMC ptp4l returned to read-only. The 01:00Z cycle ran on the BMC discipline throughout. The pulse
...
carry the excursions in their signed statements and fail the timing profile, as they should. k3's own clock logger (`epoch_stream`) shows the excursion exactly: PHC − system held 37.000 s ± 150 ns until 00:33Z, then fell
...
| aggregator start after the datagram | listener parsed, verified, wrote the record and logged before the main thread ran | **6–8 ms** after receipt (runs 4–11); measured cause: the first Ed25519 verification in a process costs 6.0 ms on k3 (library initialization), later ones 0.45 ms; a thread holding the interpreter lock after signalling delays the wake up to the 5 ms switch interval | verification warmed before the instant, the listener hands the datagram over and stops, main thread persists the record, switch interval 0.5 ms: **0.87 ms** after receipt (run 12) |
