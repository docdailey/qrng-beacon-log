...
**Spec:** [`PROTOCOL.md`](PROTOCOL.md) (normative) · **Product contract:** [`NOTBEFORE.md`](NOTBEFORE.md) (the seed \(V\), the labeled derive layer and decision contracts; draft 0.5) · **CLI:** `pip install notbefore` ([`cli/`](cli/) — `verify|value|seed|shuffle|split|sample|assign|id|range|bytes|plan|execute|timestamp|explain|pin|diff-transcript|checkpoint`, pinned vendored verifier with pinned TSA trust roots; **how to use it: [`USAGE.md`](USAGE.md)**) · **Transparency log:** [`TLOG.md`](TLOG.md) (RFC 6962 tree + C2SP checkpoints, `tlog.py`; draft 2) · **Claims discipline:** [`CLAIMS.md`](CLAIMS.md) ·
